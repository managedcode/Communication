...
public class RailwayOrientedProgrammingTests
{
    ...
    public void Map_ChainedOperations_ShouldWork()
    {
        ...
        finalResult.Value.Should().Be("13");
    }
    ...
    public void Map_WithFailure_ShouldShortCircuit()
    {
        ...
        finalResult.Problem!.Detail.Should().Be("Initial failure");
    }
    ...
    public void Bind_ChainedOperations_ShouldWork()
    {
        ...
        // Act
        var finalResult = result
            .Bind(x => x > 0 ? Result<int>.Succeed(x * 2) : Result<int>.Fail("Value must be positive", "Value must be positive"))
    ...
    public void Bind_WithFailure_ShouldShortCircuit()
    {
        ...
        // Act
        var finalResult = result
            .Bind(x => Result<int>.Fail("Bind failure", "Bind failure"))
            .Bind(x => Result<string>.Succeed(x.ToString()));

        // Assert
        finalResult.IsSuccess.Should().BeFalse();
        finalResult.Problem!.Detail.Should().Be("Bind failure");
    }
    ...
    public void Tap_ShouldExecuteSideEffectWithoutChangingResult()
    {
        ...
        // Assert
        tapResult.Should().Be(result); // Same reference
        tapResult.Value.Should().Be(5); // Value unchanged
        sideEffectValue.Should().Be(10); // Side effect executed
    }
    ...
    public void ComplexChain_ShouldWorkCorrectly()
    {
        ...
        var result = Result<string>.Succeed(input)
            ...
            .Bind(x => int.TryParse(x, out var number) 
                ? Result<int>.Succeed(number) 
                : Result<int>.Fail("Not a valid number", "Not a valid number"))
            .Tap(x => log.Add($"Parsed to: {x}"))
            .Map(x => x * 2)
            .Tap(x => log.Add($"Doubled to: {x}"))
            .Bind(x => x > 200 
                ? Result<string>.Succeed($"Large number: {x}") 
                : Result<string>.Succeed($"Small number: {x}"))
            .Tap(x => log.Add($"Final result: {x}"));

        // Assert
        result.IsSuccess.Should().BeTrue();
        result.Value.Should().Be("Large number: 246");
        log.Should().Equal(
            "Starting with: 123",
            "Parsed to: 123",
            "Doubled to: 246",
    ...
    public void ComplexChain_WithFailure_ShouldShortCircuit()
    {
        ...
        result.Problem!.Detail.Should().Be("Not a valid number");
        log.Should().Equal("Starting with: abc"); // Only first tap executed
    }
    ...
    public void Try_WithFailingOperation_ShouldReturnFailure()
    {
        ...
        result.Problem!.Detail.Should().Contain("not-a-number");
    }
    ...
    public void ResultTry_WithFailingAction_ShouldReturnFailure()
    {
        ...
        result.IsSuccess.Should().BeFalse();
        result.Problem.Should().NotBeNull();
        result.Problem!.Detail.Should().Be("Test failure");
    }
}