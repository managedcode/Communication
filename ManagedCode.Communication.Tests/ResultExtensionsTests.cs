...
public class ResultExtensionsTests
{
    ...
    public void Bind_WithFailedResult_ShouldNotExecuteNext()
    {
        ...
        bound.IsFailed.Should().BeTrue();
        bound.Problem?.Title.Should().Be("Error");
    }
    ...
    public void Else_WithFailedResult_ShouldReturnAlternative()
    {
        ...
        // Assert
        alternative.IsSuccess.Should().BeTrue();
    }
    ...
    public void Map_WithFailedResult_ShouldPropagateFailure()
    {
        ...
        // Assert
        mapped.IsFailed.Should().BeTrue();
        mapped.Problem?.Title.Should().Be("Error");
    }
    ...
    public void BindGeneric_WithSuccessResult_ShouldChainOperations()
    {
        ...
        var bound = result.Bind(x => x > 5 
            ? Result<string>.Succeed($"Value is {x}") 
            : Result<string>.Fail("Value too small"));
    ...
    public void Ensure_WithFailingPredicate_ShouldFail()
    {
        ...
        // Assert
        ensured.IsFailed.Should().BeTrue();
        ensured.Problem?.Title.Should().Be("Value too small");
    }
    ...
    public void Match_WithResultT_ShouldReturnCorrectBranch()
    {
        ...
        var successValue = successResult.Match(
            ...
            onFailure: problem => $"Failed: {problem.Title}"
        );
        ...
        // Assert
        successValue.Should().Be("Success: 42");
        failedValue.Should().Be("Failed: Error");
    }
    ...
    public void ChainedOperations_ShouldShortCircuitOnFailure()
    {
        ...
        // Act
        var final = result
            .Bind(x => Result<int>.Fail("Step 1 failed"))
            .Bind(x =>
            {
                step2Executed = true;
        ...
        final.Problem?.Title.Should().Be("Step 1 failed");
        step2Executed.Should().BeFalse();
    ...
    public void ComplexPipeline_ShouldProcessCorrectly()
    {
        ...
        var result = Result<string>.Succeed(input)
            ...
            .Map(x => x * 2)
            .Bind(x => x < 100 
                ? Result<double>.Succeed(x / 2.0) 
                : Result<double>.Fail("Too large"))
            .Tap(x => Console.WriteLine($"Current value: {x}"))
            .Map(x => $"Final result: {x.ToString("F2", System.Globalization.CultureInfo.InvariantCulture)}");
        
        // Assert
        result.IsSuccess.Should().BeTrue();
        result.Value.Should().Be("Final result: 10.00");
    }
